...
import { coins, actions, buys } from "./bonus.js";
import { cards } from "./cards.js";
import { choosefromSupply, chooseBetween } from "./choice.js";
...
export let Ironworks = new cardText("Ironworks", async function(){
    ...
    if (card != null){
        gaintodiscard(card);
        if (card.checktype("Action")){
            actions(1);
        }
        if(card.checktype("Treasure")){
            coins(1);
        }
        if(card.checktype("Victory")){
            cards(1);
        }
    }
